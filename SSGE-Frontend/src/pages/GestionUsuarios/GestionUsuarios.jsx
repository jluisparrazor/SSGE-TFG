...
import { apiFetch, getCurrentRole } from '../../lib/api.js';
...
function GestionUsuarios() {
  ...
  const [usuarioViendoHistorial, setUsuarioViendoHistorial] = useState(null);
  ...
  const verAuditoria = async (usuario) => {
    setUsuarioViendoHistorial(usuario);
    setModalAbierto(true);
    setCargandoHistorial(true);
    try {
      const res = await apiFetch(`/api/auditoria/usuario/${usuario.id}`);
      const data = await res.json();
      if (!res.ok) {
      ...
      alert(err.message || 'Error cargando auditoría');
      setHistorialUsuario([]);
    } finally {
  ...
  return (
    <div className="App">
      ...
      <main className="usuarios-page-main">
        ...
        {modalAbierto && (
          <div className="modal-auditoria-overlay" onClick={() => setModalAbierto(false)}>
            <div className="modal-auditoria-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-auditoria-header">
                <h3>Actividad de {usuarioViendoHistorial?.username}</h3>
                <button className="modal-auditoria-close" onClick={() => setModalAbierto(false)}>✕</button>
              </div>
              
              <div className="modal-auditoria-body">
                {cargandoHistorial ? (
                  <p>Cargando registros...</p>
                ) : historialUsuario.length === 0 ? (
                  <p>No hay actividad reciente registrada para este usuario.</p>
                ) : (
                  <div className="usuarios-table-wrap">
                    <table className="usuarios-table">
                      <thead>
                      ...
                      <tbody>
                        {historialUsuario.map(reg => (
                          <tr key={reg.id}>
                            <td>{new Date(reg.fechaHora).toLocaleString('es-ES')}</td>
                            <td>{reg.metodo} ({reg.estadoHttp})</td>
                            <td>{reg.endpoint}</td>
